Shader "DistanceGradient/Terrain/LowPoly/5TerrainLowPoly"
{
	Properties
	{
		...
		_SplitTop ("Top Split Distance", Float) = 40.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2g vert (vertexData v)
			{
				...
				useSmooth += lerp(_Smoothness4, _SmoothnessTop, _BlendTop * smoothstep(_Split3 * (1 + _NoiseMultiplier), _SplitTop * (1 + _NoiseMultiplier), o.currentDistance)) * step(_Split3 * (1 + _NoiseMultiplier), o.currentDistance) * step(o.currentDistance, _SplitTop * (1 + _NoiseMultiplier));
			...
			fixed4 frag (g2f i) : SV_Target
			{
				...
				c += lerp(albedo4, albedoTop, _BlendTop * smoothstep(_Split3 * (1 + _NoiseMultiplier), _SplitTop * (1 + _NoiseMultiplier), i.currentDistance)) * step(_Split3 * (1 + _NoiseMultiplier), i.currentDistance) * step(i.currentDistance, _SplitTop * (1 + _NoiseMultiplier));
