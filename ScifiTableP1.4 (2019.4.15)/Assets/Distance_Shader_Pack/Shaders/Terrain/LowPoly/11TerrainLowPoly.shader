Shader "DistanceGradient/Terrain/LowPoly/11TerrainLowPoly"
{
	Properties
	{
		...
		_SplitTop ("Top Split Distance", Float) = 100.0
		_BlendTop ("Top Blend Factor", Range(0,1)) = 0.5

		// Tenth Texture Public Variables

		_Tint10 ("Tenth Tint", Color) = (1, 1, 1, 1)
		_Texture10 ("Tenth Texture", 2D) = "white" {}
		_Tex10TileX ("Tenth X Tiling", Float) = 1.0
		_Tex10TileY ("Tenth Y Tiling", Float) = 1.0
		_Tex10TileZ ("Tenth Z Tiling", Float) = 1.0
		_Tex10BlendSharpness ("Tenth Blend Sharpness", Range(0.001, 5)) = 1.0
		[Gamma] _Metallic10 ("Tenth Metallic", Range(0, 1)) = 0
		_Smoothness10 ("Tenth Smoothness", Range(0, 1)) = 0.5
		
		// Split Variables For Between the Ninth and Tenth Textures

		_Split9 ("Ninth Split Distance", Float) = 90.0
		_Blend9 ("Ninth Blend Factor", Range(0,1)) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _Tex10TileY;
			float _Tex10TileZ;
			float _Tex10BlendSharpness;
			float _Metallic10;
			float _Smoothness10;

				// Ninth Split Public Variables
			...
			v2g vert (vertexData v)
			{
				...
				useSmooth += lerp(_Smoothness9, _Smoothness10, _Blend9 * smoothstep(_Split8 * (1 + _NoiseMultiplier), _Split9 * (1 + _NoiseMultiplier), o.currentDistance)) * step(_Split8 * (1 + _NoiseMultiplier), o.currentDistance) * step(o.currentDistance, _Split9 * (1 + _NoiseMultiplier));
				useSmooth += lerp(_Smoothness10, _SmoothnessTop, _BlendTop * smoothstep(_Split9 * (1 + _NoiseMultiplier), _SplitTop * (1 + _NoiseMultiplier), o.currentDistance)) * step(_Split9 * (1 + _NoiseMultiplier), o.currentDistance) * step(o.currentDistance, _SplitTop * (1 + _NoiseMultiplier));
			...
			fixed4 frag (g2f i) : SV_Target
			{
				...
				// Get Pixel Color for Tenth Texture

				float3 albedo10 = GetTriplanar(_Texture10, i, _Tex10TileX, _Tex10TileY, _Tex10TileZ, _Tex10BlendSharpness).rgb * _Tint10.rgb;
				albedo10 = DiffuseAndSpecularFromMetallic(albedo10, _Metallic10, specularTint, oneMinusReflectivity);
				...
				c += lerp(albedo9, albedo10, _Blend9 * smoothstep(_Split8 * (1 + _NoiseMultiplier), _Split9 * (1 + _NoiseMultiplier), i.currentDistance)) * step(_Split8 * (1 + _NoiseMultiplier), i.currentDistance) * step(i.currentDistance, _Split9 * (1 + _NoiseMultiplier));
				c += lerp(albedo10, albedoTop, _BlendTop * smoothstep(_Split9 * (1 + _NoiseMultiplier), _SplitTop * (1 + _NoiseMultiplier), i.currentDistance)) * step(_Split9 * (1 + _NoiseMultiplier), i.currentDistance) * step(i.currentDistance, _SplitTop * (1 + _NoiseMultiplier));
